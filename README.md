...
A sleek, portable, and zero-configuration Spotify playlist downloader that extracts and saves Spotify tracks as high-quality MP3s by matching audio on YouTube Music and downloading via `yt-dlp`. 

This application runs fully independently without requiring any Spotify Developer API credentials, Premium subscriptions, or complicated user configurations.
...
1. **Clone the Repository**:
   ```bash
   git clone https://github.com/djael-ml/jptdwnld.git
   cd jptdwnld
   ```
...
- **Downloader**: Paste the Spotify playlist URL/ID, select the output folder, and start the extraction.
...
```bash
python main.py --playlist "SPOTIFY_PLAYLIST_URL_OR_ID" --output "C:\Path\To\Save\Music" --quality 320
```

#### CLI Parameters:
- `--playlist`, `-p` (Required): Spotify playlist URL or its unique ID.
